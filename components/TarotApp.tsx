...
export default function TarotApp(){
  ...
  async function generateSummary(cards,pats){
    ...
    try{
      // Call our own server-side proxy — API key stays on the server, never in the browser
      var res=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2000,system:smartPrompt,messages:[{role:"user",content:"问卜者的问题："+fullQuestion+"\n牌阵："+spread.label+"\n抽到的牌："+cardsList}]})});
      var data=await res.json();
      if(!res.ok||data.error)throw new Error(data.error||"API error (status "+res.status+")");
      var raw=(data.text||"").trim().replace(/```json|```/g,"");
